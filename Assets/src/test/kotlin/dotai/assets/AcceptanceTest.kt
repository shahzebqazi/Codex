...
import dotai.assets.model.Theme
...
class AcceptanceTest {
    ...
    @Test
    fun `full pipeline produces at least 50 assets`() {
        assertTrue(allAssets.size >= 50, "Expected at least 50 assets, got ${allAssets.size}")
    }
    ...
    fun `light theme icons use white background`() {
        ...
        for (icon in lightIcons) {
            val cornerPixel = Color(icon.image!!.getRGB(0, 0), true)
            assertTrue(cornerPixel.red >= 250 && cornerPixel.green >= 250 && cornerPixel.blue >= 250,
                "Light icon ${icon.name} corner should be near-white, got RGB(${cornerPixel.red},${cornerPixel.green},${cornerPixel.blue})")
        }
    }

    @Test
    fun `primary accent color appears in dark theme icons`() {
        val primaryColor = config.colors.primary
        val darkIcons = allAssets.filter {
            it.format == AssetFormat.PNG && it.type == dotai.assets.model.AssetType.ICON
        ...
        for (icon in darkIcons) {
            ...
            for (y in 0 until img.height) {
                for (x in 0 until img.width) {
                    ...
                    if (colorDistance(pixel, primaryColor) < 30) {
                        foundPrimary = true
                        break
                    }
                }
            ...
            assertTrue(foundPrimary, "Dark icon ${icon.name} should contain primary accent color")
        }
    }
    ...
    fun `CSS export contains all brand variables`(@TempDir tempDir: File) {
        ...
        val requiredVars = listOf(
            "--dotai-primary", "--dotai-bg-dark", "--dotai-bg-surface",
            "--dotai-fg-primary", "--dotai-fg-muted", "--dotai-font-display",
            "--dotai-font-mono", "--dotai-tracking"
        )
